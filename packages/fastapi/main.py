...
@app.post("/simulate")
def simulate(input: SimulationRequest):
    # Flatten relevant persona sections
    p = input.persona
    summary = " ".join([
        " ".join(p.get("interests", [])),
        " ".join(p.get("personality_traits", [])),
        " ".join(p.get("frequent_topics", [])),
        " ".join(p.get("top_hashtags", [])),
        " ".join(p.get("likes_analysis", {}).get("liked_topics", [])),
    ])
    ...
    for q in input.questions:
        option_embeddings = model.encode(q.options, convert_to_tensor=True)
